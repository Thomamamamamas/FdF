...
{
	if (z <= fdf->smallest)
		return (hex_to_rgb(fdf->colorset.color1));
	else if (z < fdf->medium)
		return (hex_to_rgb(fdf->colorset.color2));
	else if (z >= fdf->medium && z < fdf->largest)
		return (hex_to_rgb(fdf->colorset.color3));
	else if (z >= fdf->largest)
		return (hex_to_rgb(fdf->colorset.color4));
	return (hex_to_rgb(fdf->colorset.color_white));
}
