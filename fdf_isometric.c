...
/*   Updated: 2021/12/18 18:25:14 by tcasale          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector3	**get_cartesian_coord(char **height_array, t_parse *parser)
{
	...
	while (y < parser->y)
	{
		...
		while (x < parser->x[y])
		{
			vector3_tmp.x = x * GRID_SIZE;
			vector3_tmp.y = y * GRID_SIZE;
...
t_vector3	**cartesian_to_iso(t_vector3 **coord, t_parse *parser)
{
	...
	while (y < parser->y)
	{
		...
		while (x < parser->x[y])
		{
			coord_iso[y][x].x = (coord[y][x].x - coord[y][x].y) + (MARGIN * parser->y);
			coord_iso[y][x].y = ((coord[y][x].y + coord[y][x].x)) + (MARGIN * parser->y);
			x++;
		}
		y++;
	}
